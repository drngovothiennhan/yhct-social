...
function cleanId(value: string): string {
  ...
  if (!result || result.length > 240 || !/^[A-Za-z0-9_-]+$/.test(result)) {
    throw new Error('AI_KNOWLEDGE_INVALID_SOURCE');
  }
  return result;
}
...
export async function syncKnowledgeSourceWithDeps(
  ...
  if (
    existing?.providerDocumentId
    && existing.providerDocumentId !== uploaded.providerDocumentId
    && deps.deleteFromFileSearch
  ) {
    try {
      await deps.deleteFromFileSearch(existing.providerDocumentId);
    } catch {
      // New manifest is already authoritative; stale-provider cleanup can be retried on a later sync/removal.
    }
  }
...
async function readApprovedDriveFile(driveFileId: string): Promise<{
  ...
  const metadataResponse = await driveFetch(
    `files/${encodeURIComponent(sourceId)}?fields=id,name,mimeType,modifiedTime,parents&supportsAllDrives=true`,
    token,
  );
  ...
  if (metadata.mimeType === 'application/vnd.google-apps.document') {
    ...
    contentResponse = await driveFetch(
      `files/${encodeURIComponent(sourceId)}/export?mimeType=${encodeURIComponent(uploadMimeType)}`,
      token,
    );
  } else {
  ...
  if (bytes.byteLength === 0 || bytes.byteLength > 20 * 1024 * 1024) throw new Error('AI_DRIVE_FILE_SIZE_INVALID');
  return { metadata, bytes, uploadMimeType };
}

async function uploadBytesToFileSearch(input: {
  bytes: Uint8Array;
  mimeType: string;
  title: string;
}): Promise<{ providerDocumentId: string }> {
  const { GoogleGenAI } = await import('@google/genai');
  const ai = new GoogleGenAI({ apiKey: requiredEnv('GEMINI_API_KEY') });
  const store = requiredEnv('GEMINI_FILE_SEARCH_STORE');
  let operation = await ai.fileSearchStores.uploadToFileSearchStore({
    fileSearchStoreName: store,
    file: new Blob([input.bytes], { type: input.mimeType }),
...
export async function syncDriveSource(
  ...
  return syncKnowledgeSourceWithDeps(
    { driveFileId: metadata.id, title: metadata.name, contentHash },
    actor,
    {
      async readManifest(sourceId) {
        ...
        if (
          ...
          || typeof data.providerDocumentId !== 'string'
          || data.status !== 'ready'
        ) return null;
        return {
...
export async function removeKnowledgeSource(sourceId: string, actor: KnowledgeActor): Promise<void> {
  ...
  const { adminDb } = await import('./firebase-admin');
  const db = adminDb();
  const ref = db.collection('aiKnowledgeSources').doc(safeId);
