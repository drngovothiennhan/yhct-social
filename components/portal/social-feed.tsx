...
export function SocialFeed() {
  ...
  const loadFirst = useCallback(async () => {
    ...
    setError(null);
    try {
      const page = await loadFeedPage({ filter });
      setPosts(page.posts);
      setCursor(page.cursor);
      setHasMore(page.hasMore);
    } catch (next) {
      setError(next instanceof Error ? next.message : 'Không thể tải bảng tin.');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-4">
      <SocialComposer onPublished={() => setRefreshKey((value) => value + 1)} />
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {([
          ...
          <button key={value} type="button" onClick={() => setFilter(value)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold ${filter === value ? 'bg-slate-900 text-white' : 'border border-slate-200 bg-white text-slate-600'}`}>{label}</button>
        ))}
      </div>
